...
Mac App 采用项目优先的桌面工作台：按工作目录管理项目，再在项目下创建和切换会话。
桌面端新建 Agent 时默认在内嵌 Shell 中直接运行 Codex、Claude Code 等原生 CLI；需要消息卡片、
工具过程和结构化审批时，仍可在启动面板显式选择 `Chat UI · 结构化对话`。上表的默认形态是
daemon/协议在调用方未指定轨道时的默认值。
