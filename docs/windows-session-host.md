...
PTY 的 mock replay/terminal-fence 覆盖在 daemon suite 中运行；Windows x64/arm64 CI 还会以 runner-local 临时签名副本经生产 loader 运行完整 Session Host 验收：临时 HOME/LOCALAPPDATA/repo、structured daemon graceful/exact force-kill 后同一 host/agent PID+FILETIME、连续 replay、offline approval/question、PTY snapshot/input/resize、显式 Job tree kill 与 host force-kill。测试后恢复 unsigned CI artifact；正式签名 release artifact 再重复同一验收。Unix supervisor 的 runtime-image migration 测试在 Windows 上仍明确为 `unsupported`，它不是这条独立 Session Host 路径的验收依据。
