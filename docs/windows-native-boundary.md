...
- Every pull request builds and executes the Node-API smoke suite on `windows-latest` (x64) and the unconditional `windows-11-arm` job (arm64). It stages an **unsigned** artifact only for CI evidence, raw-addon execution, and the negative production-loader smoke. The production loader must reject that artifact with `unsigned`; it is never packed into the daemon or published. The job then signs an in-place test copy with a runner-local trusted self-signed certificate solely to run the full Session Host acceptance suite through the production loader, restores the raw binary and unsigned manifest, and only then uploads the unsigned artifact.
