...
function AccountEditDialog({ account, apiProtocolsSupported, apiValidationSupported, onClose, onSaved }: { account: JsonObject; apiProtocolsSupported: boolean; apiValidationSupported: boolean; onClose: () => void; onSaved: () => void }) {
  const { t } = useLocale();
  ...
  const [baseUrl, setBaseUrl] = useState(text(profile["baseUrl"], account["apiProfileError"] ? "" : accountApiProtocolDefaults(initialProtocol).baseUrl));
  const [model, setModel] = useState(text(profile["model"], account["apiProfileError"] ? "" : accountApiProtocolDefaults(initialProtocol).model));
  const [apiKey, setApiKey] = useState("");
  const initialCapabilities = record(profile["modelCapabilities"]);
  const [modelCapabilities, setModelCapabilities] = useState(() => modelCapabilityDraft(initialCapabilities));
  const capabilitiesChanged = apiValidationSupported && JSON.stringify(modelCapabilities) !== JSON.stringify(modelCapabilityDraft(initialCapabilities));
  ...
  const apiSettingsChanged = (apiProtocolsSupported && protocol !== initialProtocol)
    ...
    || Boolean(apiKey.trim()) || capabilitiesChanged || Boolean(account["apiProfileError"]);
  ...
  const save = async (): Promise<void> => {
    ...
    setError(undefined);
    try {
      if (apiProfile && !connectionLocked && apiValidationSupported) parseModelCapabilities(modelCapabilities, initialCapabilities, protocol);
      if (apiProfile && connectionLocked && nextName !== initialName) {
        ...
        const result = await window.prospero.accountAction({ type: "agent.account.api.configure", requestId: crypto.randomUUID(), accountId, name: nextName, provider: accountApiProvider(protocol), protocol, baseUrl: nextBaseUrl, model: nextModel, ...(capabilitiesChanged ? { modelCapabilities: parseModelCapabilities(modelCapabilities, initialCapabilities, protocol) } : {}), ...(apiKey.trim() ? { apiKey: apiKey.trim() } : {}) });
  ...
  return <Dialog open onOpenChange={(open) => { if (!open && !busyRef.current) onClose(); }}><DialogContent className="sm:max-w-lg" showCloseButton={!busy} closeLabel={t("关闭", "Close")} aria-busy={busy}><DialogHeader><DialogTitle>{t("编辑账号", "Edit account")}</DialogTitle><DialogDescription>{account["apiProfileError"] ? t("原配置无法读取。请重新填写服务地址和模型；API Key 留空会保留现有凭据。", "The saved profile could not be read. Re-enter the service URL and model; leave API Key empty to retain the existing credential.") : apiProfile ? connectionLocked ? t("当前存在活动会话，仅可修改显示名称。", "Only the display name can be changed while sessions are active.") : apiProtocolsSupported ? t("修改名称和 API 连接设置。API Key 留空会保留现有凭据。", "Update the name and API connection. Leave API Key empty to keep the current credential.") : t("当前 daemon 使用旧版 API Profile；修改连接设置时需要重新输入 API Key。", "This daemon uses legacy API profiles. Re-enter the API Key when changing connection settings.") : t("修改这个独立 CLI 账号在 Prospero 中的显示名称。", "Change the display name for this isolated CLI account.")}</DialogDescription></DialogHeader>{apiProfile && connectionLocked && <p className="security-note" role="status">{t("结束活动会话后，才能更新协议、API 地址、模型或 API Key。", "End active sessions before updating the protocol, API URL, model, or API Key.")}</p>}<form className="grid gap-4" onSubmit={(event) => { event.preventDefault(); void save(); }}><FieldGroup><Field><FieldLabel htmlFor="account-edit-name">{t("显示名称", "Display name")}</FieldLabel><Input id="account-edit-name" maxLength={80} disabled={busy} value={name} onChange={(event) => setName(event.target.value)} aria-invalid={Boolean(error)} aria-describedby={error ? "account-edit-error" : undefined} autoFocus /></Field>{apiProfile && <>{apiProtocolsSupported && <Field><FieldLabel htmlFor="account-edit-protocol">{t("API 协议", "API protocol")}</FieldLabel><NativeSelect id="account-edit-protocol" disabled={busy || connectionLocked || protocols.length < 2} value={protocol} onChange={(event) => changeProtocol(event.target.value as AccountApiProtocol)}>{protocols.map((item) => <NativeSelectOption key={item} value={item}>{accountApiProtocolLabel(item)}</NativeSelectOption>)}</NativeSelect><FieldDescription>{accountApiEngineLabel(protocol)} engine · {protocol === "openai_responses" ? t("OpenAI Responses API 及兼容服务", "OpenAI Responses API and compatible services") : protocol === "openai_chat_completions" ? t("OpenAI Chat Completions 兼容服务", "OpenAI Chat Completions-compatible services") : t("Anthropic Messages API 及兼容服务", "Anthropic Messages API and compatible services")}</FieldDescription></Field>}<Field><FieldLabel htmlFor="account-edit-base-url">{t("API 地址", "Base URL")}</FieldLabel><Input id="account-edit-base-url" type="url" inputMode="url" maxLength={2_000} spellCheck={false} disabled={busy || connectionLocked} value={baseUrl} onChange={(event) => setBaseUrl(event.target.value)} /></Field><Field><FieldLabel htmlFor="account-edit-model">{t("模型", "Model")}</FieldLabel><Input id="account-edit-model" maxLength={300} spellCheck={false} disabled={busy || connectionLocked} value={model} onChange={(event) => setModel(event.target.value)} /></Field><Field><FieldLabel htmlFor="account-edit-api-key">API Key</FieldLabel><Input id="account-edit-api-key" type="password" maxLength={8_192} autoComplete="new-password" disabled={busy || connectionLocked} value={apiKey} onChange={(event) => setApiKey(event.target.value)} placeholder={apiProtocolsSupported ? t("留空以保留现有凭据", "Leave empty to keep the current credential") : t("修改连接设置时必须重新输入", "Required when changing connection settings")} /></Field>{apiValidationSupported && <ModelCapabilitiesFields value={modelCapabilities} onChange={setModelCapabilities} protocol={protocol} disabled={busy || connectionLocked} />}</>}</FieldGroup>{error && <div ref={errorRef} id="account-edit-error" className="inline-error" role="alert" tabIndex={-1}>{error}</div>}<DialogFooter><Button type="button" variant="outline" disabled={busy} onClick={onClose}>{t("取消", "Cancel")}</Button><Button type="submit" disabled={busy || legacyCredentialMissing || !name.trim() || (apiProfile && !connectionLocked && (!baseUrl.trim() || !model.trim()))}>{busy && <Spinner data-icon="inline-start" />}{busy ? t("保存中…", "Saving…") : t("保存", "Save")}</Button></DialogFooter></form></DialogContent></Dialog>;
}
...
export function AccountsPane({ snapshot, onOpenSession }: { snapshot: DesktopSnapshot; onOpenSession: (id: string, session?: SessionInfo) => void }) {
  ...
  return <div className="page accounts-page">
    ...
    <div className="provider-strip">{providers.map((provider) => {
      const configured = snapshot.accounts.filter((account) => accountEngine(account) === provider.id).length;
    ...
    {currentEditingAccount && <AccountEditDialog account={currentEditingAccount} apiProtocolsSupported={apiProtocolsSupported} apiValidationSupported={apiValidationSupported} onClose={() => setEditingAccount(undefined)} onSaved={() => { setActionError(undefined); setNotice(t("账号设置已保存", "Account settings saved")); refresh(); }} />}
  </div>;
}
