...
export function ModelSwitcher({ session, account }: { session: SessionInfo; account: JsonObject | undefined }) {
  ...
  const running = ["running", "starting", "waiting_approval", "waiting_input"].includes(session.status);
  const unavailable = !supported || session.status === "starting" || ["failed", "cancelled", "exited", "stopped"].includes(session.status);
  const busyBlocked = running && session.agent !== "codex";
  const load = async (refresh = false): Promise<void> => {
    ...
    try {
      ...
      if (mounted.current && token === request.current) setError(displayError(reason));
    } finally {
      if (mounted.current && token === request.current) setLoading(false);
    }
  };
  const save = async (): Promise<void> => {
    if (busy.current || !model || unavailable || busyBlocked || loading) return;
    busy.current = true;
    setSaving(true);
    setError(undefined);
    try {
      const selectedEffort = accountCapabilities(account).reasoningEffort ? supportedModelEffort(model, effort) : undefined;
  ...
  return <>
    <Button className="session-model-chip" variant="outline" size="sm" disabled={unavailable} title={current || t("切换模型", "Switch model")} onClick={() => { setOpen(true); setQuery(""); setNotice(undefined); void load(); }}>
    ...
    <Dialog open={open} onOpenChange={(value) => { if (!busy.current) setOpen(value); }}>
      <DialogContent className="session-model-dialog" showCloseButton={!saving} closeLabel={t("关闭", "Close")} aria-busy={loading || saving}>
        <DialogHeader><DialogTitle>{t("模型与推理强度", "Model and reasoning effort")}</DialogTitle><DialogDescription>{busyBlocked ? t("当前轮次结束后可切换模型。", "Model changes are available after the current turn.") : t("选择会话下一轮使用的模型。", "Choose the model for the next turn.")}</DialogDescription></DialogHeader>
        ...
        {accountCapabilities(account).reasoningEffort && <label className="model-effort-field"><span>{t("推理强度", "Reasoning effort")}</span><NativeSelect aria-label={t("推理强度", "Reasoning effort")} value={supportedModelEffort(model, effort) ?? ""} onChange={(event) => setEffort(event.target.value)} disabled={saving || loading || !model?.supportedEfforts.length}><NativeSelectOption value="">{t("使用模型默认值", "Use model default")}</NativeSelectOption>{model?.supportedEfforts.map((value) => <NativeSelectOption key={value} value={value}>{value}</NativeSelectOption>)}</NativeSelect></label>}
        <DialogFooter><Button variant="outline" disabled={saving} onClick={() => setOpen(false)}>{t("取消", "Cancel")}</Button><Button disabled={saving || loading || !model || busyBlocked || unavailable} onClick={() => void save()}>{saving ? t("正在切换…", "Switching…") : t("应用到下一轮", "Apply to next turn")}</Button></DialogFooter>
      </DialogContent>
    </Dialog>
  </>;
}
