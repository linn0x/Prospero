...
const EventCard = memo(function EventCard(props: EventProps) {
  ...
  if (kind === "conversation.turn") return <ConversationTurn event={event} projectRoot={projectRoot} renderEvent={(entry, activity) => activity && entry.kind === "reasoning" ? <div className="chat-reasoning"><MarkdownContent value={text(entry.text)} onError={onError} projectRoot={projectRoot} /></div> : activity && ["tool.start", "tool.end"].includes(text(entry.kind)) ? <ToolCard compact event={entry} openOutput={openOutput} /> : <EventCard {...props} event={entry} isResolved={Boolean(activity)} />} />;
