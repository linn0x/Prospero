...
const EventCardBody = memo(function EventCardBody(props: EventProps) {
  ...
  if (kind === "subagent.started" || kind === "subagent.updated") {
    const subagent = record(event.subagent); const id = text(subagent.id, text(event.subagentId)); const label = text(subagent.name, id); const summary = terminalPlainText(text(event.summary, text(subagent.preview)));
