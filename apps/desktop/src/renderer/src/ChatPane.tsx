import { ConversationTurn } from "./chat/ConversationTurn";
import { usePagedTimeline } from "./use-paged-timeline";
import { TimelineBodyDialog } from "./TimelineBodyDialog";
import { conversationTurns } from "./chat/conversation-turns";
...
import { Bot, Check, ChevronDown, CircleAlert, Clock3, Code2, Copy, ExternalLink, FileImage, ListChecks, Paperclip, Send, ShieldAlert, Target, Trash2, UserRound, X, Workflow } from "lucide-react";
...
import { array, displayError, isMissingSessionError, reportError, number, record, text } from "./state";
import { useLocale } from "./locale";
import { terminalPlainText } from "./terminal-text";
import { SourceSelector } from "./model-sources/SourceSelector";
import { selectedSourceRoute, type SourceRuntimeAgent, type SourceSelection } from "./model-sources/source-state";
import { useModelSources } from "./model-sources/use-model-sources";
import {
...
function ToolCard({ event, openOutput, compact = false }: { event: JsonObject; openOutput: (id: string, tool: string) => void; compact?: boolean }) {
  ...
  const summary = terminalPlainText(text(event.summary, state === "running" ? t("正在执行…", "Running…") : t("执行完成", "Completed")));
  if (compact) return <div className="trajectory-tool">
    ...
    <p className="terminal-output-text">{summary}</p>
...
type EventProps = { event: JsonObject; projectRoot?: string | undefined; isResolved: boolean; sessionId: string; onError: (message?: string) => void; openOutput: (id: string, tool: string) => void; openSubagent: (id: string, label: string) => void };

function CrossModelChildLauncher({ session, onCreated, onError }: { session: SessionInfo; onCreated: (id: string) => void; onError: (message: string) => void }) {
  const { t } = useLocale();
  const supported = typeof window.prospero.createCrossModelChild === "function";
  const sourceState = useModelSources(supported);
  const [open, setOpen] = useState(false);
  const [agent, setAgent] = useState<SourceRuntimeAgent>("codex");
  const [selection, setSelection] = useState<SourceSelection>();
  const [task, setTask] = useState("");
  const [busy, setBusy] = useState(false);
  const choice = selectedSourceRoute(sourceState.sources, selection, agent);
  if (!supported || session.kind !== "structured") return null;
  const launch = async (): Promise<void> => {
    if (!choice || !selection || !task.trim() || busy) return;
    setBusy(true);
    try {
      const result = await window.prospero.createCrossModelChild(session.id, { sourceId: selection.sourceId, routeId: selection.routeId, revision: selection.revision, agent, task: task.trim() });
      const id = text(result["sessionId"]);
      if (!id) throw new Error(t("子 Agent 创建没有返回会话 ID", "Child agent creation did not return a session id"));
      setTask(""); setOpen(false); onCreated(id);
    } catch (reason) { onError(reportError(reason)); } finally { setBusy(false); }
  };
  return <>
    <Button variant="outline" size="sm" disabled={sourceState.loading} onClick={() => setOpen(true)}><Workflow data-icon="inline-start" />{t("跨模型子 Agent", "Cross-model child")}</Button>
    <Dialog open={open} onOpenChange={(next) => { if (!busy) setOpen(next); }}><DialogContent className="sm:max-w-2xl"><DialogHeader><DialogTitle>{t("派发跨模型子 Agent", "Dispatch a cross-model child")}</DialogTitle><DialogDescription>{t("独立模型源、独立 structured 会话，固定 YOLO 自动批准；完成摘要回传当前会话。", "Independent model source and structured session with YOLO auto-approval. Its summary returns here.")}</DialogDescription></DialogHeader><div className="grid gap-4"><NativeSelect value={agent} disabled={busy} aria-label={t("子 Agent 类型", "Child agent runtime")} onChange={event => { setAgent(event.target.value as SourceRuntimeAgent); setSelection(undefined); }}><NativeSelectOption value="codex">Codex</NativeSelectOption><NativeSelectOption value="claude">Claude Code</NativeSelectOption><NativeSelectOption value="opencode">OpenCode</NativeSelectOption></NativeSelect><SourceSelector sources={sourceState.sources} loading={sourceState.loading} error={sourceState.error} value={selection} agent={agent} onChange={setSelection} onRefresh={() => void sourceState.refresh()} disabled={busy} /><InputGroup><InputGroupTextarea value={task} maxLength={65_536} rows={6} aria-label={t("子任务", "Child task")} placeholder={t("描述要交给另一个模型完成的独立任务…", "Describe an independent task for the other model…")} onChange={event => setTask(event.target.value)} /></InputGroup>{choice && <p className="text-xs text-muted-foreground">{t("目标：", "Target: ")}{choice.source.name} / {choice.route.model} · {t("YOLO 自动批准", "YOLO auto-approval")}</p>}</div><DialogFooter><Button variant="outline" disabled={busy} onClick={() => setOpen(false)}>{t("取消", "Cancel")}</Button><Button disabled={busy || !choice || !task.trim()} onClick={() => void launch()}>{busy && <Spinner data-icon="inline-start" />}{t("派发子任务", "Dispatch child")}</Button></DialogFooter></DialogContent></Dialog>
  </>;
}
const EventCardBody = memo(function EventCardBody(props: EventProps) {
  const { event, isResolved, sessionId, onError, openOutput, openSubagent, projectRoot } = props;
  const { t, status } = useLocale();
  const kind = text(event.kind);
  if (kind === "conversation.turn") return <ConversationTurn event={event} projectRoot={projectRoot} renderEvent={(entry, activity) => activity && entry.kind === "reasoning" && entry.hasMore !== true ? <div className="chat-reasoning"><MarkdownContent value={text(entry.text)} onError={onError} projectRoot={projectRoot} /></div> : activity && ["tool.start", "tool.end"].includes(text(entry.kind)) ? <ToolCard compact event={entry} openOutput={openOutput} /> : <EventCard {...props} event={entry} isResolved={Boolean(activity)} />} />;
  if (kind === "user.message") {
    const attachments = array(event.attachments).map(record);
    return <Message align="end"><MessageAvatar><Avatar><AvatarFallback><UserRound className="size-4" /></AvatarFallback></Avatar></MessageAvatar><MessageContent><MessageHeader>{t("你", "You")}</MessageHeader><Bubble align="end" variant="secondary"><BubbleContent className="grid gap-2 whitespace-pre-wrap">{text(event.text) && <MarkdownContent value={text(event.text)} onError={onError} projectRoot={projectRoot} />}{attachments.length > 0 && <AttachmentGroup>{attachments.map((attachment, index) => <Attachment state="done" size="sm" key={text(attachment.id, String(index))}><AttachmentMedia><FileImage /></AttachmentMedia><AttachmentContent><AttachmentTitle>{text(attachment.name, t(`图片 ${String(index + 1)}`, `Image ${String(index + 1)}`))}</AttachmentTitle><AttachmentDescription>{text(attachment.mimeType, t("图片", "Image"))}</AttachmentDescription></AttachmentContent></Attachment>)}</AttachmentGroup>}</BubbleContent></Bubble></MessageContent></Message>;
  }
  if (kind === "assistant.text") return <Message><MessageAvatar><Avatar><AvatarFallback><Bot className="size-4" /></AvatarFallback></Avatar></MessageAvatar><MessageContent><MessageHeader>Agent</MessageHeader><Bubble variant="ghost"><BubbleContent><MarkdownContent value={text(event.text)} onError={onError} projectRoot={projectRoot} /></BubbleContent></Bubble></MessageContent></Message>;
  if (kind === "reasoning") return <Collapsible><CollapsibleTrigger render={<Button variant="ghost" size="sm" />}><ChevronDown data-icon="inline-start" />{t("思考过程", "Reasoning")}</CollapsibleTrigger><CollapsibleContent><div className="chat-reasoning mt-2 max-h-96 overflow-auto rounded-lg bg-muted p-3 text-muted-foreground"><MarkdownContent value={text(event.text)} onError={onError} projectRoot={projectRoot} /></div></CollapsibleContent></Collapsible>;
  ...
  if (kind === "subagent.started" || kind === "subagent.updated") {
    const subagent = record(event.subagent); const id = text(subagent.id, text(event.subagentId)); const label = text(subagent.name, id); const role = text(subagent.role); const summary = terminalPlainText(text(event.summary, text(subagent.preview))); const crossModel = role.includes("跨模型") || role.includes("Cross-model");
    return <Card size="sm"><CardHeader><CardTitle className="flex items-center gap-2"><Bot className="size-4" />{crossModel ? t("跨模型子 Agent", "Cross-model child") : t("子 Agent", "Subagent")}</CardTitle><CardDescription>{label}</CardDescription></CardHeader><CardContent className="flex flex-col items-start gap-3">{summary && <p className="text-sm text-muted-foreground whitespace-pre-wrap">{summary}</p>}{id && !crossModel && <Button variant="outline" size="sm" onClick={() => openSubagent(id, label)}><ExternalLink data-icon="inline-start" />{t("查看执行详情", "View execution details")}</Button>}{crossModel && <p className="text-xs text-muted-foreground">{t("作为独立会话运行，可从侧栏查看完整过程。", "Runs as an independent session; use the sidebar for its full transcript.")}</p>}</CardContent><CardFooter><Badge variant="secondary">{status(text(subagent.status, text(event.status, "active")))}</Badge></CardFooter></Card>;
  }
  if (kind === "trajectory.record") return <Alert><Bot /><AlertTitle>{text(event.title)}</AlertTitle><AlertDescription><MarkdownContent value={text(event.detail)} onError={onError} projectRoot={projectRoot} /><Badge variant={text(event.phase) === "failed" ? "destructive" : "secondary"}>{status(text(event.phase))}</Badge></AlertDescription></Alert>;
...
const EventCard = memo(function EventCard(props: EventProps) {
  const { t } = useLocale();
  return <><EventCardBody {...props} />{props.event.hasMore === true && Boolean(props.event.bodyRecordId) && !["tool.start", "tool.end"].includes(text(props.event.kind)) && <Button variant="outline" size="sm" onClick={() => props.openOutput(text(props.event.bodyRecordId), t("完整内容", "Full content"))}><ExternalLink data-icon="inline-start" />{t("查看完整内容", "Read full content")}</Button>}</>;
});
const TimelineItem = memo(function TimelineItem({ item, ...props }: TimelineItemProps) {
  const kind = text(item.event.kind);
  return <MessageScrollerItem key={item.key} messageId={item.key} data-chat-key={item.key} scrollAnchor={kind === "user.message" || kind === "conversation.turn"}><EventCard event={item.event} {...props} /></MessageScrollerItem>;
});
...
export function ChatPane({ session, account, onOpenGoal, trajectoryHost, onMissingSession }: { session: SessionInfo; account?: JsonObject | undefined; onOpenGoal?: () => void; trajectoryHost?: HTMLDivElement | null; onMissingSession?: (id: string) => void }) {
  const { t } = useLocale();
  const paged = usePagedTimeline(session.id, session.historyMode === "paged");
  const [bodyDetail, setBodyDetail] = useState<{ id: string; title: string; sessionId: string }>();
  ...
  const [legacyTimeline, setTimeline] = useState(() => accumulator.current!.snapshot()); const cursor = useRef<number | undefined>(undefined);
  const timeline = paged.enabled ? paged.timeline : legacyTimeline;
  ...
  const [operationError, setOperationError] = useState<string>(); const [legacyConnectionError, setConnectionError] = useState<string>();
  const connectionError = paged.enabled ? paged.state.error : legacyConnectionError;
  ...
  const displayItems = useMemo(() => conversationTurns(timeline.items, timeline.resolutions), [timeline]);
  ...
  const navigationPoints = useMemo(() => displayItems.filter((item) => item.event.kind === "conversation.turn"), [displayItems]);
  ...
  const jumpToLatest = useCallback((): void => { if (paged.enabled) void paged.controller.latest(); jumpToLatestRef.current = true; selectHistoryCursor(null); }, [paged.controller, paged.enabled, selectHistoryCursor]);
  const handleTimelineScroll = useCallback((event: UIEvent<HTMLDivElement>): void => {
    const element = event.currentTarget;
    if (paged.enabled) { if (!isChatViewportNearEnd(element)) paged.controller.freeze(); return; }
  ...
  }, [paged.controller, paged.enabled, selectHistoryCursor]);
  useEffect(() => {
  ...
  }, [onMissingSession, paged.enabled, selectHistoryCursor, session.id]);
  ...
  const openOutput = useCallback(async (id: string, tool: string): Promise<void> => {
    if (paged.enabled) { setBodyDetail({ id, title: tool, sessionId: session.id }); return; }
    ...
    try {
      ...
      setDetail({ title, content: terminalPlainText(text(result.output, t("（无输出）", "(No output)"))), truncated: result.truncated === true });
      setOperationError(undefined);
    } catch (reason) {
      if (detailRequest.current !== request) return;
      setDetail(undefined);
  ...
  const openSubagent = useCallback(async (id: string, label: string): Promise<void> => {
    ...
    try {
      ...
      if (detailRequest.current !== request) return;
      const lines = collapseChatEventHistory(array(result.events).map(record)).map((event) => {
        const body = terminalPlainText(text(event.text, text(event.summary, text(event.message, text(event.detail)))));
        return `${text(event.kind)}${body ? `\n${body}` : ""}`;
      });
