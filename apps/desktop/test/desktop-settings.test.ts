...
describe("desktop settings input", () => {
  it("normalizes valid values", () => {
    expect(desktopSettingsPatch({ daemonBackend: "rust", terminalFontFamily: "  Geist Mono  ", terminalFontSize: 14, daemonBind: "127.0.0.1" }, "darwin")).toEqual({
      daemonBackend: "rust",
  ...
  it("rejects unknown and malformed values", () => {
    ...
    expect(() => desktopSettingsPatch({ daemonBackend: "sqlite" }, "darwin")).toThrow("运行时后端设置无效");
