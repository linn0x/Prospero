...
impl Store {
    ...
    pub(crate) fn enqueue_message(
        ...
        if row.text.is_empty() || row.text.len() > 64 * 1024 {
    ...
    pub(crate) fn message_queue(&self, session_id: &str) -> Result<Vec<QueuedRow>> {
        ...
        let mut statement = self.connection.prepare(
            "SELECT queue_id,kind,text,created_at FROM agent_message_queue \
    ...
    pub(crate) fn message_queues(&self) -> Result<Vec<(String, Vec<QueuedRow>)>> {
        let mut statement = self.connection.prepare(
            "SELECT q.session_id,q.queue_id,q.kind,q.text,q.created_at \
    ...
    pub(crate) fn guide_message(&mut self, session_id: &str, queue_id: &str) -> Result<()> {
        ...
        crate::database::validate_id(queue_id)?;
        let transaction = self.connection.transaction()?;
        let (text, created_at): (String, i64) = transaction
            .query_row(
                "SELECT text,created_at FROM agent_message_queue \
        ...
        transaction.execute(
            "INSERT INTO agent_message_queue(session_id,queue_id,position,kind,text,created_at) \
             VALUES(?1,?2,?3,'guide',?4,?5)",
            rusqlite::params![session_id, queue_id, front, text, created_at],
        )?;
        transaction.commit()?;
        Ok(())
    }
