...
fn schema_indexes_survive_reopen() {
    ...
    let indexed: i64 = connection
        ...
        .unwrap();
    assert_eq!(indexed, 5);
    let automation_columns: i64 = connection
        .query_row(
        ...
        .unwrap();
    assert_eq!(automation_columns, 1);
...
#[test]
fn v8_database_is_migrated_forward_to_current_schema() {
...
#[test]
fn v9_database_is_migrated_forward_to_current_schema() {
...
#[test]
fn v10_database_is_migrated_forward_to_current_schema() {
...
#[test]
fn v11_database_is_migrated_forward_to_current_schema() {
...
#[test]
fn v12_database_is_migrated_forward_to_current_schema() {
...
#[test]
fn v13_database_is_migrated_forward_to_current_schema() {
    ...
    let database = directory.path().join("prospero.sqlite");
    {
        ...
        connection
            .execute(
                "INSERT INTO session_heads \
                 (id,created_at,lifecycle,revision,payload) \
                 VALUES('sess-a',1,'active',1,json('{\"workspace\":\"/w\",\"agent\":\"claude\",\"kind\":\"structured\",\"title\":\"A\",\"status\":\"idle\"}'))",
                [],
            )
            .unwrap();
        connection
            .execute(
                "INSERT INTO agent_runs(session_id,agent,active,approval_policy,permission_mode,turn,native_id) \
                 VALUES('sess-a','claude',1,'manual','default',1,'native-a')",
                [],
            )
            .unwrap();
        connection
            .execute(
                "INSERT INTO agent_message_queue(session_id,queue_id,position,kind,text,created_at) \
                 VALUES('sess-a','q1',0,'queue','legacy row',10)",
                [],
            )
            .unwrap();
    }
    ...
    let (text, attachments): (String, String) = connection
        .query_row(
            "SELECT text,attachments FROM agent_message_queue WHERE queue_id='q1'",
            [],
            |row| Ok((row.get(0)?, row.get(1)?)),
        )
        .unwrap();
    assert_eq!(text, "legacy row");
    assert_eq!(attachments, "[]");
    // New-style rows with attachments are writable.
    connection
        .execute(
            "INSERT INTO agent_message_queue(session_id,queue_id,position,kind,text,created_at,attachments) \
             VALUES('sess-a','q2',1,'queue','with image',11,'[{\"mimeType\":\"image/png\",\"dataB64\":\"AAAA\"}]')",
            [],
        )
        .unwrap();
    let count: i64 = connection
        .query_row(
            "SELECT count(*) FROM agent_message_queue WHERE json_array_length(attachments)>0",
            [],
            |row| row.get(0),
        )
        .unwrap();
...
#[test]
fn v14_database_is_migrated_forward_to_current_schema() {
    ...
    let database = directory.path().join("prospero.sqlite");
    {
        ...
        connection
            .execute(
                "INSERT INTO session_heads \
                 (id,created_at,lifecycle,revision,payload) \
                 VALUES('sess-m',1,'active',1,json('{\"workspace\":\"/w\",\"agent\":\"claude\",\"kind\":\"structured\",\"title\":\"A\",\"status\":\"idle\"}'))",
                [],
            )
            .unwrap();
        connection
            .execute(
                "INSERT INTO agent_runs(session_id,agent,active,approval_policy,permission_mode,turn,native_id) \
                 VALUES('sess-m','claude',1,'manual','default',1,'native-m')",
                [],
            )
            .unwrap();
    }
    ...
    let (model, effort): (Option<String>, Option<String>) = connection
        .query_row(
            "SELECT model,effort FROM agent_runs WHERE session_id='sess-m'",
            [],
            |row| Ok((row.get(0)?, row.get(1)?)),
        )
        .unwrap();
    ...
    // New selections are writable and survive a reopen.
    connection
        .execute(
            "UPDATE agent_runs SET model='opus-latest',effort='high' WHERE session_id='sess-m'",
            [],
        )
        .unwrap();
    drop(connection);
    ...
    let store = Store::open(directory.path()).unwrap();
    let connection = raw(&directory);
    let (model, effort): (String, String) = connection
        .query_row(
            "SELECT model,effort FROM agent_runs WHERE session_id='sess-m'",
            [],
            |row| Ok((row.get(0)?, row.get(1)?)),
...
#[test]
fn v15_database_is_migrated_forward_to_current_schema() {
    ...
    connection
        .execute(
            r#"INSERT INTO session_heads
             (id,created_at,lifecycle,revision,payload)
             VALUES('sess-codex',3,'active',1,json('{"workspace":"/w","agent":"codex","kind":"structured","title":"C","status":"idle"}'))"#,
            [],
        )
        .unwrap();
    connection
        .execute(
            "INSERT INTO agent_runs(session_id,agent,active,approval_policy,permission_mode,turn,native_id,model,effort,account_id) \
             VALUES('sess-codex','codex',1,'manual','default',0,NULL,NULL,NULL,NULL)",
            [],
        )
        .unwrap();
    connection
        .execute(
            r#"INSERT INTO session_heads
             (id,created_at,lifecycle,revision,payload)
             VALUES('sess-deepseek',4,'active',1,json('{"workspace":"/w","agent":"deepseek","kind":"structured","title":"D","status":"idle"}'))"#,
            [],
        )
        .unwrap();
    connection
        .execute(
            "INSERT INTO agent_runs(session_id,agent,active,approval_policy,permission_mode,turn,native_id,model,effort,account_id,agent_preset) \
             VALUES('sess-deepseek','deepseek',1,'manual','default',0,NULL,NULL,NULL,NULL,'reviewer')",
            [],
        )
        .unwrap();
    let preset: Option<String> = connection
        .query_row(
            "SELECT agent_preset FROM agent_runs WHERE session_id='sess-acct'",
            [],
            |row| row.get(0),
        )
        .unwrap();
