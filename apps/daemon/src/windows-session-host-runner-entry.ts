...
const signedAcceptance = process.env["PROSPERO_WINDOWS_SIGNED_SESSION_HOST_TEST"] === "1";
const stateDirectory = process.env["PROSPERO_WINDOWS_SESSION_HOST_STATE_DIRECTORY"];

function writeAcceptanceDiagnostic(stage: "entry_loaded" | "entry_failed", error?: unknown): void {
  if (!signedAcceptance || typeof stateDirectory !== "string" || stateDirectory.length === 0) return;
  const message = error instanceof Error
    ? error.message.replace(/[\r\n|]+/g, " ").replace(/\s+/g, " ").trim().slice(0, 320)
    : undefined;
  try {
    writeFileSync(join(stateDirectory, "host.test-diagnostic.json"), JSON.stringify({
      version: 1,
      stage,
      ...(message ? { message } : {}),
    }));
