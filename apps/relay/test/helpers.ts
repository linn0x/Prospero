...
import type { EphemeralStore, RouteStore, SnapshotCredential } from "../src/store.js";
...
export class MemoryRouteStore implements RouteStore {
  ...
  async applyDeviceSnapshot(routeId: string, generation: number, credentials: SnapshotCredential[]): Promise<RouteSnapshot | null> {
    this.ensure(); const route = this.routes.get(routeId); if (route === undefined || route.disabledAt !== null || generation < route.generation) return null;
...
export class MemoryEphemeralStore implements EphemeralStore {
  ...
  readonly presence = new Map<string, string>(); readonly tickets = new Map<string, StreamTicket>(); readonly listeners = new Set<(event: RelayEvent) => void>();
  ...
  async createTicket(ticket: StreamTicket, _ttl: number): Promise<void> { this.ensure(); if (this.tickets.has(ticket.ticket)) throw new Error("collision"); this.tickets.set(ticket.ticket, { ...ticket }); }
  async consumeTicket(ticket: string): Promise<StreamTicket | null> { this.ensure(); const value = this.tickets.get(ticket); this.tickets.delete(ticket); return value === undefined ? null : { ...value }; }
