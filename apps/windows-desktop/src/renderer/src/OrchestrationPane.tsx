    ...
    <div className="orchestration-layout">
      ...
      <main className="run-detail">{selectedRun ? <>
        ...
        <Tabs value={runView} onValueChange={(value) => setRunView(value ?? "board")} className="run-views"><TabsList variant="line"><TabsTrigger value="board"><Columns3 />{t("看板", "Board")}</TabsTrigger><TabsTrigger value="dag"><Network />DAG</TabsTrigger><TabsTrigger value="timeline"><Clock3 />{t("时间线", "Timeline")}</TabsTrigger></TabsList>
          ...
          <TabsContent value="dag" className="run-view-content"><div className="dag-canvas">{runTasks.map((task, index) => <div className={`dag-node-card ${text(task["status"])}`} key={text(task["id"])} style={{ gridColumn: `${String((index % 3) * 2 + 1)} / span 1`, gridRow: `${String(Math.floor(index / 3) * 2 + 1)}` }}><span className={`status-dot ${text(task["status"])}`} /><div><strong>{text(task["title"])}</strong><small>{status(text(task["status"]))} · {array(task["deps"]).length} {t("个依赖", "deps")}</small></div></div>)}{runTasks.length === 0 && <Empty><EmptyHeader><EmptyMedia variant="icon"><Network /></EmptyMedia><EmptyTitle>{t("暂无 DAG 节点", "No DAG nodes")}</EmptyTitle><EmptyDescription>{t("添加任务后依赖图会显示在这里。", "Add tasks to populate the dependency graph.")}</EmptyDescription></EmptyHeader></Empty>}</div></TabsContent>
