...
import {
  Activity, Archive, ArrowDownAZ, ArrowRight, Bot, Boxes, BookOpen, CheckCircle2, ChevronRight, CircleAlert,
  CircleStop, Clock3, Copy, FileDiff, Folder, FolderKanban, FolderOpen, FolderPlus, GitBranch,
  LayoutDashboard, ListChecks, LoaderCircle, Mail, MessageSquare, MoreHorizontal,
  ...
  return <Sidebar collapsible="icon" className="prospero-sidebar">
    ...
    <SidebarContent>
      ...
      <SidebarGroup><SidebarGroupLabel>{t("置顶", "Pinned")}</SidebarGroupLabel><SidebarGroupContent><SidebarMenu>
        {pinned.map((session) => { const attention = (session.pendingPermissions ?? 0) + (session.pendingQuestions ?? 0); return <SidebarMenuItem key={session.id}><SidebarMenuButton size="lg" tooltip={sessionLabel(session)} onClick={() => onOpenSession(session.id)}><StatusMark status={attention ? "waiting_input" : session.status} /><span className="flex min-w-0 flex-col gap-0.5"><span className="truncate text-xs font-medium">{sessionLabel(session)}</span><span className="truncate text-[10px] font-normal text-sidebar-foreground/45">{session.agent} · {attention ? t("需要输入", "Needs input") : status(session.status)} · {relativeTime(session.createdAt, language)}</span></span></SidebarMenuButton><SidebarMenuAction showOnHover aria-label={t(`取消置顶 ${sessionLabel(session)}`, `Unpin ${sessionLabel(session)}`)} onClick={() => onTogglePin(session.id)}><PinOff /></SidebarMenuAction></SidebarMenuItem>; })}
