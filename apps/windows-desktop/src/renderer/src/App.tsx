...
function WorkspacePane({ snapshot, activeId, openIds, onActivate, onClose, onNewSession, onTogglePin, focus }: { snapshot: DesktopSnapshot; activeId: string | undefined; openIds: string[]; onActivate: (id: string) => void; onClose: (id: string) => void; onNewSession: (project?: string) => void; onTogglePin: (id: string) => void; focus: boolean }) {
  const { t, status } = useLocale();
  const [showContext, setShowContext] = useState(true);
  // 专注模式下强制收起,退出后恢复用户自己的选择。
...
export function App({ snapshot }: { snapshot: DesktopSnapshot }) {
  ...
  // 专注模式:侧栏和上下文栏一起让位,只留下当前会话本身。
  // 会话是这个客户端的主体,其余都是导航 —— 需要沉进去的时候它们只是噪音。
  ...
  return <SidebarProvider open={sidebarOpen && !focus} onOpenChange={setSidebarOpen} style={{ "--sidebar-width": "15rem", "--sidebar-width-icon": "3.5rem" } as React.CSSProperties} className="prospero-shell">
