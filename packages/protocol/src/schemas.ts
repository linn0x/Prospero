...
export const ModelSourceActionSchema = z.discriminatedUnion("kind", [
  ...
  z.object({ kind: z.literal("routes.set"), ...modelSourceWrite, routes: z.array(ModelSourceRouteSchema.omit({ id: true }).extend({ id: modelSourceId.optional() })).min(1).max(500) }).strict(),
